...
internals.AccountManager.prototype.findByEmail = function(emails)
{
    ...
    return this.find({'profile.emails': emails});
};


/**
 * Creates a new entry for  
 * @param {model.Auth} authCriteria - the Auth model which was populated from auth provider
 */
internals.AccountManager.prototype.createAccountAndAuth = function(account, auth)
{
    var promise = promiseutils.createPromise(function(resolve, reject) {
        this.findByEmail(account.profile.emails[0])
        ...
        .then(function(accountModel) {
            ...
            this.getAuthManager().add(auth)
            .then(function(authModel) {
                var auth = authModel.toJSON();
                auth.accountObject = accountModel.toJSON();
                resolve(auth);
            });
        }.bind(this))
