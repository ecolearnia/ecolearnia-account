...
var SYSID_COLUMN = 'uuid';
//var SYSID_COLUMN = '_id';
 ...
 *
 */
internals.AccountManager = function(persistenceProvider)
{
...
internals.AccountManager.prototype.SYSID_COLUMN = function()
{
  return SYSID_COLUMN;
}
...
internals.AccountManager.prototype.add = function(resource, options)
{
    ...
    .then(function(account) {
        ...
        {
            resource.auth.account = account[SYSID_COLUMN];
 ...
 */
internals.AccountManager.prototype.removeAssociatedAuths = function(accounts)
{
    var self = this;
        ...
        if (accounts && accounts.length > 0) {
            ...
            accounts.forEach (function (element, index){
              var authAccountFK = self.getAuthManager().ACCOUNT_FK();
                var authCriteria = QueryHelper.createComparisonOp('=', authAccountFK, element[SYSID_COLUMN]);
 ...
 */
internals.AccountManager.prototype.touch = function(pk)
{
    // @todo - check if emails is an array, if it is use 'in' operator
    var criteria = QueryHelper.createComparisonOp('=', this.accountProvider.primaryKey_, pk);
    var setLastLogin = {
...
internals.AccountManager.createAccount = function(displayName, role, kind, status, email)
{
    return {
        kind: kind,
        roles: [role],
...
internals.getManager = function(persistenceProviderName)
{
    if (!internals.managerInstance)
    {
        ...
        internals.managerInstance = new internals.AccountManager(persistenceProvider);
    }
