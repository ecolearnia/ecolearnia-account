...
internals.AuthManager.prototype.findFromCredentials = function(authCriteria)
{
    var promise = promiseutils.createPromise(function(resolve, reject) {
        ...
        var auth;
        var criteria = {
            'authSource': authCriteria.authSource, 
            'authId': authCriteria.authId, 
        };
        this.find(criteria)
        .then(function(hitAuthModel) {
            auth = hitAuthModel.toJSON();
            var accountCriteria = { _id: hitAuthModel.account };
            return this.getAccountManager.find(accountCriteria)
        }.bind(this))
        .then(function(accountModel) {
            auth.account = accountModel.toJSON();
            resolve(auth);
        })
