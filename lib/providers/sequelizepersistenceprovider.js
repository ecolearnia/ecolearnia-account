...
internals.SequelizePersistenceProvider.prototype.find = function(criteria, options)
{
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		this.Model_.findOne({where: sequelizeWhere, raw: raw})
 ...
 */
internals.SequelizePersistenceProvider.prototype.updatePartial = function(criteria, resource, options)
{
	var swLog = new logger.StopwatchLog(this.logger_, 'update');
	swLog.start({criteria: criteria, options: options});
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		...
		this.Model_.update(setObject, {where: sequelizeWhere}, function(error, doc) {
			swLog.stop();
...
internals.SequelizePersistenceProvider.prototype.remove = function(criteria, options)
{
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		...
		.catch(function(error){
			swLog.stop();
			var exception = Exception.wrapError(error);
			logger.Logger.logError(this.logger_, exception, 'Error when deleting from DB', logMeta);
			throw exception;
		}.bind(this))
	}.bind(this));
