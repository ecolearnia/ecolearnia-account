...
var CriteriaTranslatorMongo = require('../utils/criteriatranslatormongo').CriteriaTranslatorMongo;
		...
		resourceModel.save(function(error){
		    ...
		    if(error) {
		        ...
		        reject(exception);
					...
					// @todo: change so it returns JSON object
...
internals.MongoPersistenceProvider.prototype.find = function(criteria, options)
{
	...
	this.logger_.trace({ mongoCriteria: mongoCriteria}, 'find:native-criteria');
...
internals.MongoPersistenceProvider.prototype.query = function(criteria, options)
{
	...
	this.logger_.trace({ mongoCriteria: mongoCriteria}, 'query:native-criteria');
 ...
 */
internals.MongoPersistenceProvider.prototype.updatePartial = function(criteria, resource, options)
{
	var swLog = new logger.StopwatchLog(this.logger_, 'update');
	swLog.start({criteria: criteria, options: options});
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		...
		var setObject = { $set: resource };

		this.Model_.findOneAndUpdate(mongoCriteria, setObject, { multi: true }, function(error, doc) {
			swLog.stop();
			this.logger_.debug({result: doc});
